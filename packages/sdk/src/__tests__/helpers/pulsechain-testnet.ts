...
import { readTestnetKey } from './testnet-key-store.js';
 ...
 *   1. `options.privateKey` (explicit).
 *   2. `PULSECHAIN_TESTNET_PRIVATE_KEY` environment variable.
 *   3. Persisted mnemonic at `packages/sdk/.pulsechain-testnet.local.json`
 *      (gitignored; generated by `scripts/gen-testnet-key.ts`).
 *   4. Fresh random key — only useful for smoke-testing helpers, since it has
 *      no funds and no faucet claim is likely to succeed (IP cooldown).
 */
export function createTestnetContext(options: {
  ...
  const persisted = envKey ? null : readTestnetKey();
